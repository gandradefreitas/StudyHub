...
def carregar_dashboard(usuario_id):
    ...
    segundos_estudo_hoje = (
        obter_segundos_estudados_hoje(usuario_id)
    )

    segundos_provas_hoje = (
        obter_segundos_provas_hoje(usuario_id)
    )
    ...
    meta_estudo_segundos = (
        meta_estudo * 60
    )

    estudo_atual_formatado = formatar_duracao(
        segundos_hoje
    )

    estudo_meta_formatado = formatar_duracao(
        meta_estudo_segundos
    )

    proxima_prova = obter_proxima_prova(
        usuario_id
    )
    ...
    tempo_estudado_total = formatar_duracao(
        segundos_totais
    )
    ...
    porcentagem_estudo = min(
        porcentagem_estudo,
        100
    )

    porcentagem_questoes = min(
        porcentagem_questoes,
        100
    )
    ...
    return {
        ...
        "metas": {
            ...
            "estudo_atual_formatado":
                estudo_atual_formatado,

            "estudo_meta_formatado":
                estudo_meta_formatado,
